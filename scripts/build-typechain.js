...
async function buildTypechain() {
  ...
  mergeEvents({ src: 'OpenTermLoanRefinancer.abi.json', dst: 'OpenTermLoanV2.abi.json' })
  mergeEvents({ src: 'FixedTermLoanRefinancer.abi.json', dst: 'FixedTermLoan.abi.json' })
  mergeEvents({ src: 'FixedTermLoanRefinancer.abi.json', dst: 'FixedTermLoanV6.abi.json' })
  ...
  // V4's template must also decode it: the migrator fires during the v4->v5 upgrade tx,
  // before the v5 template exists (same pattern as PoolPermissionManagerSet on V3).
